...
public class MainActivity extends AppCompatActivity {
    ...
    private double refVal = 0.0, realVal = 0.0, realRef = 0.0, memVal = 0.0;
    ...
    private int count = 0, count_down = 0;
    private double calMax = 0.0, calMin = 0.0, differ, min = 0, max = 0;
    private Socket mSocket;
    private JSONObject jObject, data;
    private String api = "";
    private String current_view, main_v = "Main", free_v = "Free", qs_v = "QS";
    private ImageView free_im;
...
//
            ...
            try {
                ...
                if ((state.equals("STOP") || state.equals("DONE")) && current_view.equals(main_v)) {
                    ...
                    calMax = data.getDouble(Config.TAG_CAL_MAX);
                    calMin = data.getDouble(Config.TAG_CAL_MIN);
                    differ = calMax - calMin;
                    if (calMax > 340) {
                        max = 360 - calMax;
                    } else {
                        max = 20;
        ...
        public void handleMessage(Message msg) {
            if (msg.what == COUNT_DOWN) {
                ...
                try {
                    ...
                    realVal = data.getDouble(Config.TAG_REAL_VAL);
                    refVal = data.getDouble(Config.TAG_REF_VAL) * 10;
//                    realRef = data.getDouble(Config.TAG_REAL_REF);
                    time_tv.setText(String.valueOf(((int) (refVal))));
                    memVal = data.getDouble(Config.TAG_MEM_VAL) * Config.BASE_VAL;
                    System.out.println("mem " + data.getDouble(Config.TAG_REAL_VAL));
                    ...
                    if (realVal > calMax + max) {
                        realVal = calMax + max;
                    } else if (realVal < calMin - min) {
                        realVal = calMin - min;
                    }

                    if (realVal > calMax) {
                        realVal = (realVal - calMax) * 20 / max;
                        System.out.println("mem max " + (int) realVal);
                        mem_pb.setProgress(1000);
                        max_pb.setProgress((int) realVal);
                        min_pb.setProgress(0);
                    } else if (realVal < calMin) {
                        realVal = (calMin - realVal) * 20 / min;
                        System.out.println("mem min " + (int) realVal);
                        mem_pb.setProgress(0);
                        max_pb.setProgress(0);
                        min_pb.setProgress((int) realVal);
                    } else {
                        System.out.println("mem morna " + (int) ((realVal - calMin) * 1000 / differ));
                        mem_pb.setProgress((int) ((realVal - calMin) * 1000 / differ));
                        max_pb.setProgress(0);
                        min_pb.setProgress(0);
                    }
